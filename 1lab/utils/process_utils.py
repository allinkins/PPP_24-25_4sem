...
class ProcessUtils:
    ...
    def send_signal(pid: int, sig: str) -> str:
        ...
        if platform.system() == "Windows" and sig == "SIGKILL":
            try:
                proc = psutil.Process(pid)
                proc.terminate()
                return f"SIGKILL (terminate) отправлен процессу с PID {pid}"
            except psutil.NoSuchProcess:
                return f"Процесс с PID {pid} не найден"
            except psutil.AccessDenied:
                return f"Доступ к завершению процесса с PID {pid} запрещён"
        ...
        try:
            ...
            return f"Недостаточно прав для отправки сигнала процессу с PID {pid}"
        except Exception as e:
